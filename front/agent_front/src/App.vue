...
const messages = ref<ChatMessage[]>([
  {
    id: `m-${Date.now()}`,
    role: 'assistant',
    content: '你好，我是智能研究助手。你可以直接提问，我会根据意图自动走快速回答或完整研究链路。',
  },
])
...
const pushProgress = (message: string, node?: string) => {
  let msg = message.trim()
  if (!msg) return
  if (node && nodeLabelMap[node]) {
    msg = nodeLabelMap[node]
  }
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        if (event.type === 'status' || event.type === 'phase' || event.type === 'route') {
          pushProgress(event.message || '', event.node)
          renderStatusText()
        }
        if (event.type === 'final') {
  ...
  <div class="chat-shell">
    <aside class="chat-sidebar">
      <div class="sidebar-brand">
        <p class="brand-badge">Research Assistant</p>
        <h1>智能研究系统</h1>
