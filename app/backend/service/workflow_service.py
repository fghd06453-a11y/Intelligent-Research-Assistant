...
from app.mult_agents.main import build_agents, build_checkpointer, build_memory_manager
from app.mult_agents.state import create_initial_state
...
class WorkflowService:
    ...
    def _ensure_initialized(self) -> None:
        if self._initialized:    #防止重复初始化，
            return
        with self._lock:     #防止多个线程同时初始化
            if self._initialized:
                return
            base_config = AppConfig.from_env()
            self._memory_manager = build_memory_manager(base_config)   #构建记忆管理器，
